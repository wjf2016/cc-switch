...
Manage reusable outbound proxy servers, allowing individual providers to bind to specific proxies (falls back to direct connection if unbound). The legacy single "Global Outbound Proxy" configuration is automatically migrated into this pool upon upgrade startup.
