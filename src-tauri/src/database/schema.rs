...
impl Database {
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（新增代理服务器池表并升级使用聚合表）");
    ...
    /// v10 -> v11 迁移：
    /// 1. 新增代理服务器池表 (proxy_servers) 及其索引；
    ...
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        ...
        log::info!(
            "v10 -> v11 迁移完成：代理服务器池表已建立，且 usage_daily_rollups 已保留 request_model/pricing_model 维度"
        );
        Ok(())
    }
